...
## Error 179 — notify-test.js threw ReferenceError on `provider.shutdown()`; iOS saw "Network error: Load failed"
**Scope:** `netlify/functions/notify-test.js` — backend diagnostic endpoint. Same bug class as Error 172 but server-side.
**Build:** Discovered June 3, 2026 — user (gated dev unlock) tapped Send Test Push from in-app Settings and got the WKWebView fetch error "Network error: Load failed" instead of either a delivered push or a structured diagnostic response.
**Side:** `netlify/functions/notify-test.js`, last line of the handler before the summary return.
**Symptom:** Function ran through all 6 stages successfully (env checks, session auth, device lookup, APNs send loop) but threw `ReferenceError: provider is not defined` on the cleanup line `provider.shutdown()`. The throw escaped the handler with no response, Netlify's runtime returned a generic 500 with no CORS headers, and the iOS WKWebView surfaced the generic "Network error: Load failed" message that masks the actual cause.
**Root cause:** Build-29-era refactor split the single `provider` into `providerProd` and `providerDev` to handle the production-vs-sandbox APNs token mismatch (Error 159 family). The shutdown call at the end of the handler was never updated; the bare `provider` identifier was a leftover that the JS parser couldn't catch at compile time and Node's module loader didn't surface until the line actually executed.
**Fix:** Replaced bare `provider.shutdown()` with explicit `providerProd.shutdown()` and `providerDev.shutdown()`, each wrapped in its own try/catch so a shutdown failure on one provider cannot kill the diagnostic response. Same defensive-shutdown pattern already used in notify-pipeline.js (the `finally` block).
**Lesson:** When a refactor splits one variable into multiple, grep the whole file for the old identifier — not just the call sites that were obviously affected. The JS parser will not catch references to undeclared identifiers at compile time, and a single missed `provider.foo()` at the bottom of a long handler can break a feature for months without anyone noticing because the symptom looks like a network problem, not a code problem. Also: every cleanup call in a serverless handler should be wrapped in try/catch — the response must always send, even if teardown fails.
**Files:** `netlify/functions/notify-test.js`
**Commit:** pending
