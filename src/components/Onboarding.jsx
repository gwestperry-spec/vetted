  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "28px 20px 0", flex: 1 }}>
        <p style={{ fontFamily: "var(--font-data)", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--accent)", marginBottom: 16 }}>
          {t.appEyebrow}
        </p>
        <h1 style={{ fontFamily: "var(--font-prose)", fontSize: 32, fontWeight: 500, color: "var(--ink)", lineHeight: 1.15, margin: "0 0 12px", letterSpacing: "-0.01em" }}>
          {t.appTitle1}<span style={{ color: "var(--accent)" }}>{t.appTitleAccent}</span>{t.appTitle2}
        </h1>
        <p style={{ fontFamily: "var(--font-prose)", fontSize: 15, color: "var(--muted)", lineHeight: 1.55, margin: "0 0 32px" }}>
          {t.appTagline}
        </p>

        {/* Country selector */}
        <p style={{ fontFamily: "var(--font-data)", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 14 }}>
          {t.regionTitle}
        </p>
          ...
          {NA_COUNTRIES.map(c => (
            <button
              ...
              style={{
                ...
                background: selectedCountry === c.code ? "var(--ink)" : "var(--cream)",
                border: `1px solid ${selectedCountry === c.code ? "var(--ink)" : "var(--border)"}`,
                color: selectedCountry === c.code ? "#F4F8F0" : "var(--ink)",
                cursor: "pointer", transition: "all 150ms ease",
                display: "flex", flexDirection: "column", alignItems: "center", gap: 6,
              }}
            >
              <span style={{ fontSize: 22 }}>{c.flag}</span>
              <span style={{ fontFamily: "var(--font-data)", fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase" }}>
                {t.countries[c.code]}
              </span>
            </button>
          ))}
      ...
      {/* CTA */}
      <div style={{ padding: "28px 20px 38px" }}>
        <button
          disabled={!selectedCountry}
          onClick={onContinue}
          style={{
            width: "100%", minHeight: 56, borderRadius: 14,
            background: selectedCountry ? "var(--ink)" : "#C8D4C5",
            color: selectedCountry ? "#F4F8F0" : "rgba(244,248,240,0.5)",
            border: "none", cursor: selectedCountry ? "pointer" : "not-allowed",
            fontFamily: "var(--font-data)", fontSize: 11, fontWeight: 500,
            letterSpacing: "0.16em", textTransform: "uppercase",
            display: "flex", alignItems: "center", justifyContent: "space-between",
            padding: "0 22px", transition: "background 200ms ease",
          }}
        >
          {t.regionContinue}
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <path d="M3 9H15" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
            <path d="M10 4L15 9L10 14" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
        <p style={{ textAlign: "center", marginTop: 16, fontSize: 11, color: "var(--muted)", fontFamily: "var(--font-data)" }}>
